...
# output_dir <- "/media/winter_olympic_160/城区"
...
# 设置模式名称
# model_names <- c('GMOSRR', 'GMOSRR(EC)', 'CMA_MESO(1km)', 'CMA_MESO(3km)')
model_names <- c('CMA_MESO(3km)')
...
for (model in model_names){
  ...
  for (fhour in fhours){
    ...
    if (model == 'GMOSRR(EC)') {
      ...
      outfilename <- file.path(output_subdir, paste0(outfilename,'.png'))
    ...
    breaks <- round(seq(6, 22, by=0.4), digits=2)
    p_temp <- ggplot(out, aes(x, y, z=z)) +
      ...
      scale_fill_gradientn(name="Temp", colours=colors, limits=c(6, 22)) +
    ...
    # 设置标题
    title <- paste0("逐小时快速滚动智能网格预报(",valid_time_str,")\n",
    ...
    shademat %>% 
      ...
      plot_3d(elev_matrix, zscale = zscale, windowsize = c(1000, 1000), background = "white", 
              water = FALSE, soliddepth = -1000, wateralpha = 0)
    ...
    if (file.exists(outfilename)){
      ...
      img_colorbar <- magick::image_read(file.path(work_dir, 'data', "stadium_temp_colorbar_1.png"))
