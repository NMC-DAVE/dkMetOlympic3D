...
# * 在命令窗口中运行:
# *     R --vanilla --verbose CMD BATCH /media/kan-dai/workspace/workcode/personal/dk_winter_olympic_3D/winter_olympic_forecast_3D.R
# 实验用webgl出图, 但非常慢, 且字体不能正常显示.
#==========================================================
  ...
  for (model in model_names){
    ...
    for (fhour in fhours){
      ...
      render_snapshot(outfilename, title_text = title, instant_capture=FALSE, bring_to_front=TRUE,
                      title_size = 45, title_color = "black", title_font = "SimHei", webshot=TRUE)
    }
  }
  
  # 运行完毕后暂停一段时间
  end_time <- Sys.time()
  dt <- difftime(start_time, end_time, units="secs")
  if (dt < 120) {
    Sys.sleep(900)
  } else {
    Sys.sleep(300)
